...
function ProjectImage({ alt, slug }: { alt: string; slug: string }) {
  return (
...
function CaseStudyButton({ slug }: { slug: string }) {
  ...
  return (
    <Link
      ...
      className="flex min-h-14 w-full items-center justify-center bg-foreground px-4 py-3 text-center text-[18px] text-background transition-opacity focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-foreground"
    >
      ...
      <div className="flex flex-col">
        {PROJECTS.map((project, index) => (
          <div key={`${project.title}-${index}`} className="flex flex-col gap-10 py-10 first:pt-0 last:pb-0 lg:grid lg:grid-cols-2 lg:gap-6">
            {project.imageFirst ? (
              <>
                <ProjectImage alt={project.imageAlt} slug={project.slug} />
